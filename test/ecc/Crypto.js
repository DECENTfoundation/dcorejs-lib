import {Aes, hash, key, PrivateKey, PublicKey, Signature} from "../../src";
...
describe("ECC", function() {
    ...
    describe("Crypto", function() {
        ...
        it("Decrypt", function() {
            const aes = Aes.fromSeed("Password01");
            const d = aes.decryptHex(encrypted_key);
            assert.equal(decrypted_key, d, "decrypted key does not match");
        });

        it("Encrypt", function() {
            const aes = Aes.fromSeed("Password01");
            const d = aes.encryptHex(decrypted_key);
        ...
        it("binary_encryption", function() {
            const sender = PrivateKey.fromSeed("1");
            const receiver = PrivateKey.fromSeed("2");
            const S = sender.get_shared_secret(receiver.toPublicKey());
            const nonce = "289662526069530675";

            const ciphertext = Aes.encrypt_with_checksum(
                sender,
        ...
        it("key_checksum", function(){
            ...
            return min_time_elapsed(function(){
                const key_checksum = key.aes_checksum("password").checksum;
                assert.equal(
                    true,
        ...
        it("key_checksum with aes_private", function(done){
            this.timeout(1500);
            return min_time_elapsed(function(){
                const aes_checksum = key.aes_checksum("password");
                const aes_private = aes_checksum.aes_private;
                const key_checksum = aes_checksum.checksum;
        ...
        it("password aes_private", function() {
            ...
            const password_aes = min_time_elapsed(function () {
